...
Server.prototype._process = function(input, req, callback) {
  ...
  if (typeof self.authenticate === 'function') {
    ...
    // add the request path to the security header so that authentication functions can use this info if required
    obj.Header.Security.Path = url.parse(req.url).pathname;

    // assume there's a callback object in the authenticate call if the signature has two args
    if (self.authenticate.length === 2) {
      // new async authenticate call if arguments include a callback
      self.authenticate(obj.Header.Security, function(authenticated) {
        if (authenticated) {
          self._authenticated_process(obj, req, callback);
        } else {
          callback('Unauthorized', 401);
        }
      });
    } else {
      // original synchronous authenticate call
    ...
    // no authentication function present so just proceed
    self._authenticated_process(obj, req, callback);
  }
}

Server.prototype._authenticated_process = function(obj, req, callback) {
  var self = this,
